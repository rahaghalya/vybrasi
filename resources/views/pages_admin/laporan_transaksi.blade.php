    ...
    <div class="card mt-18">
        <table class="dtable">
            ...
            <tbody>
                ...
                <tr>
                    <td><span class="mono">{{ $trx->no_invoice }}</span></td>
                    <td class="mu">{{ \Carbon\Carbon::parse($trx->created_at)->translatedFormat('d M Y') }}</td>
                    ...
                    @endphp
                    <td class="fw">{{ $namaPelanggan }}</td>
                    
                    {{-- Cek apakah transaksi ini menggunakan kode referral --}}
                    <td class="gold" style="font-size: 13px;">
                    ...
                    <td class="fw">Rp {{ number_format($trx->total_harga, 0, ',', '.') }}</td>
...
.mono{background:#0a0a0a;border:1px solid #1e1e1e;padding:4px 8px;border-radius:6px;font-family:monospace;color:#D4A373;font-size:13px}
...
.bdg{padding:5px 12px;border-radius:20px;font-size:11px;font-weight:700;display:inline-block}
