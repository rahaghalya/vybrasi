...
{{-- Override semua background putih dari parent layout (Persis seperti di Beranda) --}}
<style>
...
<div class="wrap">
    ...
    <form action="{{ route('admin.produk.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="pad">
                <div class="frm-grid">
                    <div class="span2">
                        ...
                        <input type="text" name="nama" class="inp" placeholder="ex: Kopi Arabica Gayo" value="{{ old('nama') }}" required>
                    </div>

                    <div>
                        <label class="lbl">Kategori Produk <span class="red">*</span></label>
                        <div style="position: relative;">
                            <select name="kategori" class="inp" style="appearance: none; cursor: pointer;" required>
                                <option value="" style="color:#000;">Pilih Kategori</option>
                                <option value="biji_kopi" style="color:#000;" {{ old('kategori') == 'biji_kopi' ? 'selected' : '' }}>Biji Kopi (Roasted Beans)</option>
                                <option value="kopi_bubuk" style="color:#000;" {{ old('kategori') == 'kopi_bubuk' ? 'selected' : '' }}>Kopi Bubuk (Ground Coffee)</option>
                            </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="harga" class="inp" placeholder="Contoh: 150000" value="{{ old('harga') }}" required min="0">
                    </div>

                    <div>
                        <label class="lbl">Stok Awal <span class="red">*</span></label>
                        <input type="number" name="stok" class="inp" placeholder="Contoh: 50" value="{{ old('stok') }}" required min="0">
                    </div>

                    <div>
                        <label class="lbl">Berat Bersih (Gram) <span class="red">*</span></label>
                        <input type="number" name="berat_gram" class="inp" placeholder="Contoh: 250" value="{{ old('berat_gram') }}" required min="0">
                    </div>

                    <div class="span2">
                        <label class="lbl">Deskripsi Produk <span class="red">*</span></label>
                        <textarea name="deskripsi" class="inp" placeholder="Tuliskan deskripsi rasa, asal kopi, dan catatan lainnya..." style="min-height: 120px; resize: vertical;" required>{{ old('deskripsi') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card mt-18">
            ...
            <div class="pad">
                <label class="lbl">Foto Produk Utama <span class="red">*</span></label>
                <div class="up-area">
                    <input type="file" name="gambar_utama" id="file-upload" accept="image/png, image/jpeg, image/jpg" required>
                    <div id="upload-icon" class="up-ic"><i class="fas fa-cloud-upload-alt"></i></div>
                    <div id="upload-text" class="up-txt">Klik atau Tarik gambar ke area ini</div>
                    <div class="up-sub">Format: JPG, JPEG, PNG &mdash; Maks. 2MB</div>
                </div>
            </div>
        </div>
...
<style>
/* CSS SUPER MINIMALIS & DARK IDENTIK DENGAN BERANDA */
