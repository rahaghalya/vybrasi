    ...
    <div class="ed-right-content">
        <div class="ed-content-inner">
            ...
            @php
                ...
                $rataRating = $totalUlasan > 0 ? $produk->ulasan->avg('rating') : 0;
            @endphp
            
            <div class="ed-rating-box">
                <div class="ed-score-wrap">
                    <span class="ed-score">{{ $totalUlasan > 0 ? number_format($rataRating, 1) : '0.0' }}</span>
                    <div class="ed-stars">
                        {{-- Bintang utama dinamis (menyala sesuai rata-rata rating) --}}
                        @for($i = 1; $i <= 5; $i++) 
                            <i class="fa-solid fa-star" style="color: {{ $i <= round($rataRating) ? '#1A251C' : '#E2E8F0' }};"></i> 
                        @endfor
                    </div>
                    <span class="ed-review-count">{{ $totalUlasan }} Ulasan</span>
                </div>
                <div class="ed-rating-bars">
                    {{-- Looping dari Bintang 5 ke 1 secara dinamis --}}
                    @for($star = 5; $star >= 1; $star--)
                        @php
                            // Hitung ada berapa orang yang memberi rating bintang ini
                            $jumlahBintangIni = $produk->ulasan->where('rating', $star)->count();
                            
                            // Hitung persentasenya untuk lebar bar (mencegah pembagian dengan 0)
                            $persentaseLebar = $totalUlasan > 0 ? ($jumlahBintangIni / $totalUlasan) * 100 : 0;
                        @endphp
                        <div class="ed-bar-row">
                            <span>{{ $star }} <i class="fa-solid fa-star" style="font-size: 8px;"></i></span>
                            <div class="ed-bar-track">
                                <div class="ed-bar-fill" style="width: {{ $persentaseLebar }}%;"></div>
                            </div>
                        </div>
                    @endfor
                </div>
